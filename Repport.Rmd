---
title: 'Storm events in the United States : impact on population health and economy'
author: "Duf59"
...
Storm events in the United States : impact on population health and economy
------------------------------------------------------------------------------
...
Storms and other severe weather events can cause both public health and economic problems for communities and municipalitie.
...
By considering the total number of deaths for each type of event, we highlight the 5 most harmful events with respect to population health. Similarly, by considering the comulative economic damage, we highlight the 5 most harmful events with respect to economy. 
...
Data processing was based on the following informations :

* The [storm event database] [2], containign entries from 1950 to 2011
...
Data processing and analysis were performed using R statistical programming software and Rstudio environement.

* Version of R: 3.1.1
* Version of Rstudio : 0.98.1074
* Operating System : Windows 7, SP1, 32 bits

Besides, the following packages ware used : 
...
Because the analysis focuses on the impact of storm event on population healt and economy, we only load the variables of interest which are :
...
Prior to analysis, the dataset has to be cleaned and subseted, especially :
...
3. Informations regarding damages are split in two variables : a value (PROPDMG,CROPDMG) and an attribute (PROPDMGEXP,CROPDMGEXP). The latter is a character such as "k,h,m,.." standing for Hundred,THounsand,Million, ... or a digit that we assume to be a power of 10 (e.g. "3" stands for Thousand).

The following subsection describe in more detail the method used to adress points 2 and 3.
...
For more details informations regarding the classification of event names one can refer to the [documentation] [3] or [severe weather terminology] [5].
...
Damage value and its attribute are gathered into a single value using the function *cleanDamage* below (see comments for more details). The function is called whhen subsetting the dataset (see next step).
...
Last step of the data processing is to subset only the events included in the 48 valid event types according to the specification of NWS Directive 10-1605. Doing this we also define more explicit variable names. This results in the tidy dataset *storm.data*.
...
To this end we rank the events according to the total number of deaths (we also look at the total number of injuries) :
...
And we plot the top 5 most harmfull events :
...
The graph above shows the 5 most harmull events with respect to the number of deaths.
The most harmfull storm event is the Tornado, which also display by far the largest total injuries, followed by Heat. Numbers are shown below :
